...
import type { CampaignEvent, CampaignEventChoice } from '../core/CampaignTypes';
...
import {
  getScenarioEventDeck,
  getScenarioVPCandidates
} from '../core/ScenarioContent';
import {
...
import {
  ...
  type DonorBlocState,
  type MediaChannelId,
  type MediaChannelState
} from '../core/CampaignStrategy';
import {
  ageOppositionResearch,
  commissionOppositionResearch,
  createOppositionResearchBoard,
  getNationalResearchPressure,
  releaseOppositionHit,
  type RivalResearchFile
} from '../core/OppositionResearch';
import { buildDebateAftermath, createDebateAnswerRecord } from '../core/DebateAftermath';
import { unlockAchievement } from '../core/Achievements';
import { getCloudSaveFileName } from '../core/SteamSync';
...
export type PlayerHomeRegion = 'National' | 'Northeast' | 'South' | 'Midwest' | 'West';
export type StatePollingProjection = PollingData | PrimaryStateProjection;
export interface PrimaryFieldHistoryEntry {
  week: string;
  standings: PrimaryFieldShare[];
}
...
export interface GameState {
  ...
  scenarioId: string;
  scenarioName: string;
  scenarioElectionYear: number;

  playerName: string;
  playerHomeRegion: PlayerHomeRegion;
  budget: number;
  publicTrust: number;
  ...
  donorBlocs: DonorBlocState[];
  mediaChannels: MediaChannelState[];
  oppositionResearch: Record<string, RivalResearchFile>;
  ...
  pollingData: Record<string, StatePollingProjection>;
  primaryResults: Record<string, PrimaryStateResult>;
  primaryFieldAverages: PrimaryFieldShare[];
  primaryFieldHistory: PrimaryFieldHistoryEntry[];
  ...
  fundraiseFromBloc: (blocId: DonorBlocId) => boolean;
  investInMedia: (channelId: MediaChannelId) => boolean;
  commissionResearch: (rivalId: string) => boolean;
  releaseResearchLead: (rivalId: string, leadId: string) => boolean;
...
function computeEVTotals(states: StateElectionData[], pollingData: Record<string, StatePollingProjection>) {
  let playerEV = 0;
  let rivalEV = 0;
...
const initialState: Omit<GameState, 'initializeCampaign' | 'runSimulation' | 'setSpending' | 'buildFieldOffice' | 'deployVolunteers' | 'withdrawVolunteers' | 'deploySurrogate' | 'hireStaff' | 'selectVP' | 'advanceWeek' | 'addBudget' | 'spendBudget' | 'addMomentum' | 'resolveEvent' | 'answerDebateQuestion' | 'advanceDebate' | 'previewDebate' | 'answerConventionChoice' | 'advanceConvention' | 'courtEndorsement' | 'advanceElectionNight' | 'finalizeElectionNight' | 'fundraiseFromBloc' | 'investInMedia' | 'commissionResearch' | 'releaseResearchLead' | 'saveGame' | 'loadGame' | 'getSaveSlots' | 'setHasStarted' | 'resetGame'> = {
  ...
  donorBlocs: [] as DonorBlocState[],
  mediaChannels: [] as MediaChannelState[],
  oppositionResearch: {} as Record<string, RivalResearchFile>,
  ...
  volunteerReserve: 0,
  pollingData: {} as Record<string, StatePollingProjection>,
  primaryResults: {} as Record<string, PrimaryStateResult>,
  primaryFieldAverages: [] as PrimaryFieldShare[],
  primaryFieldHistory: [] as PrimaryFieldHistoryEntry[],
...
function buildPrimaryFieldStanding(playerName: string, projection: PrimaryStateProjection): PrimaryFieldShare[] {
  if (projection.allFieldShares?.length) {
    return projection.allFieldShares.map((entry) => (
      entry.candidateId === 'player'
  ...
  ].sort((a, b) => b.share - a.share) as PrimaryFieldShare[];
}

function isPrimaryStateProjection(poll: StatePollingProjection): poll is PrimaryStateProjection {
  return 'allFieldShares' in poll;
}
...
function buildDebateParticipants(
  debate: ActiveDebate,
  playerIssues: string[],
  rivals: RivalAI[],
  generalOpponent: RivalAI | null
): ActiveDebate['participants'] {
  const playerTagline = playerIssues.length > 0
    ? `Your campaign | ${playerIssues.slice(0, 2).join(' / ')}`
    : 'Your campaign';

  return debate.participants.map((participant) => {
    if (participant.role === 'player') {
      return { ...participant, tagline: playerTagline };
    }

    const rival = rivals.find((entry) => entry.name === participant.name)
      ?? (generalOpponent?.name === participant.name ? generalOpponent : undefined);
    if (!rival) return participant;

    return {
      ...participant,
      tagline: getRivalDebateTagline(rival)
    };
  });
}

function applyDonorEffects(
  donorBlocs: DonorBlocState[],
  donorEffects: CampaignEventChoice['donorEffects']
): DonorBlocState[] {
  if (!donorEffects || donorEffects.length === 0) {
    return donorBlocs;
  }

  return donorBlocs.map((bloc) => {
    const effect = donorEffects.find((entry) => entry.blocId === bloc.id);
    if (!effect) {
      return bloc;
    }
    ...
    return {
      ...bloc,
      relationship: clampStat(bloc.relationship + effect.relationshipDelta),
      energy: clampStat(bloc.energy + effect.energyDelta)
    };
  });
}

function applyMediaEffects(
  mediaChannels: MediaChannelState[],
  mediaEffects: CampaignEventChoice['mediaEffects']
): MediaChannelState[] {
  if (!mediaEffects || mediaEffects.length === 0) {
    return mediaChannels;
  }

  return mediaChannels.map((channel) => {
    const effect = mediaEffects.find((entry) => entry.channelId === channel.id);
    if (!effect) {
      return channel;
    }
    ...
    return {
      ...channel,
      intensity: clampStat(channel.intensity + effect.intensityDelta)
    };
  });
}

function getRapidResponseIntensity(mediaChannels: MediaChannelState[]) {
  return mediaChannels.find((channel) => channel.id === 'rapid_response')?.intensity ?? 0;
}

function normalizeOppositionResearch(
  board: Record<string, RivalResearchFile> | undefined,
  rivals: RivalAI[]
): Record<string, RivalResearchFile> {
  const defaults = createOppositionResearchBoard(rivals);

  return Object.fromEntries(
    Object.entries(defaults).map(([rivalId, baseFile]) => {
      const savedFile = board?.[rivalId];
      return [
        rivalId,
        savedFile
          ? {
              ...baseFile,
              ...savedFile,
              leads: savedFile.leads ?? baseFile.leads
            }
          : baseFile
      ];
    })
...
function applyRivalEventEffects(
  choice: CampaignEventChoice,
  rivalAIs: RivalAI[],
  generalOpponent: RivalAI | null
): { rivalAIs: RivalAI[]; generalOpponent: RivalAI | null } {
  if (!choice.rivalId) {
    return { rivalAIs, generalOpponent };
  }

  const applyToRival = (rival: RivalAI) => ({
    ...rival,
    momentum: clampStat(rival.momentum + (choice.rivalMomentumEffect ?? 0)),
    trust: clampStat(rival.trust + (choice.rivalTrustEffect ?? 0)),
    budget: Math.max(0, rival.budget + (choice.rivalBudgetEffect ?? 0))
  });

  return {
    rivalAIs: rivalAIs.map((rival) => rival.id === choice.rivalId ? applyToRival(rival) : rival),
    generalOpponent: generalOpponent && generalOpponent.id === choice.rivalId ? applyToRival(generalOpponent) : generalOpponent
  };
}
...
function getIdeologyDistance(left: PlayerDemographics, right: PlayerDemographics): number {
  return (Object.keys(left) as Array<keyof PlayerDemographics>).reduce((sum, trait) => {
    return sum + Math.abs(left[trait] - right[trait]);
  }, 0);
}

function pickSuspensionTarget(
  playerIdeology: PlayerDemographics,
  rivals: RivalAI[],
  endorsements: ActiveEndorsement[],
  ...
  const playerDistance = getIdeologyDistance(playerIdeology, withdrawingRival.ideology);
  const playerCoalition = getCandidateEndorsementSummary(endorsements, 'player');
  let bestTargetId: string | null = 'player';
  let bestScore = (1000 - playerDistance) + (playerCoalition.prestige * 6);

  for (const rival of activeTargets) {
    const distance = getIdeologyDistance(rival.ideology, withdrawingRival.ideology);
    const coalition = getCandidateEndorsementSummary(endorsements, rival.id);
    const score = (1000 - distance) + (coalition.prestige * 7) + rival.delegates + (rival.trust * 1.8);
    if (score > bestScore) {
      bestScore = score;
...
function evaluatePrimarySuspension(
  rivals: RivalAI[],
  primaryResults: Record<string, PrimaryStateResult>,
  endorsements: ActiveEndorsement[],
  playerIdeology: PlayerDemographics,
  ...
  withdrawingRival: RivalAI;
  endorsementTargetId: string | null;
  reason: string;
  supportTransfer: number;
  momentumTransfer: number;
  trustTransfer: number;
} | null {
  const activeRivals = getOrderedRivals(rivals).filter((rival) => rival.status === 'active');
  if (activeRivals.length <= 2 || currentWeek < 34) {
    return null;
  }

  const leader = activeRivals[0];
  const candidates = activeRivals.slice(1).map((rival) => {
    const winStats = getCandidateWinStats(primaryResults, rival.id, currentWeek);
    const coalition = getCandidateEndorsementSummary(endorsements, rival.id);
    const laneCrowding = activeRivals.some((other) => {
      if (other.id === rival.id) return false;
      const isCloseLane = getIdeologyDistance(other.ideology, rival.ideology) <= 78;
      const isStronger = other.delegates > rival.delegates + 80 || other.momentum > rival.momentum + 9;
      return isCloseLane && isStronger;
    });
    const delegateRatio = rival.delegates / Math.max(1, leader.delegates);
    const delegateDeficit = leader.delegates - rival.delegates;
    const factors = [
      rival.budget < 240000 ? 'cash dried up' : null,
      rival.trust < 42 ? 'elite support cracked' : null,
      rival.momentum < 32 ? 'the campaign lost altitude' : null,
      delegateRatio < 0.42 || delegateDeficit > Math.max(220, delegateTarget * 0.11) ? 'the delegate path narrowed' : null,
      currentWeek >= 42 && winStats.stateWins === 0 ? 'their home-region promise never materialized' : null,
      laneCrowding ? 'a stronger rival captured their lane' : null,
      coalition.count === 0 && currentWeek >= 40 ? 'validators stopped taking the calls' : null
    ].filter(Boolean) as string[];

    const pressureScore = factors.length
      + (rival.budget < 180000 ? 1 : 0)
      + (rival.trust < 38 ? 1 : 0)
      + (rival.momentum < 26 ? 1 : 0)
      + (winStats.recentWins === 0 && currentWeek >= 40 ? 1 : 0);
  ...
  const target = [...candidates]
    .filter((entry) => entry.pressureScore >= 3 && entry.factors.length > 0)
  ...
  const endorsementTargetId = pickSuspensionTarget(playerIdeology, rivals, endorsements, target.rival);
  return {
    withdrawingRival: target.rival,
    endorsementTargetId,
    reason: target.factors.slice(0, 2).join(' and '),
    supportTransfer: 4 + Math.min(6, target.coalition.prestige + Math.floor(target.rival.supportBase / 3)),
    momentumTransfer: target.rival.temperament === 'charismatic' ? 6 : 4,
    trustTransfer: target.rival.trust >= 50 ? 4 : 2
  };
}
...
function buildPlayerEndorsementSnapshot(
  ...
  return {
    ...
    homeRegion: state.playerHomeRegion,
    supportBase: 14,
    status: 'player'
  };
}
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  initializeCampaign: (statesData: StateElectionData[]) => {
    ...
    const calendar = CampaignDataParser.generateCalendar(state.scenarioElectionYear);
    ...
    const opponents = SimulationEngine.createPrimaryRivals(state.difficulty, state.voterParty, statesData, state.scenarioId);
    const donorBlocs = createInitialDonorBlocs(state.playerIdeology, state.playerIssues, state.voterParty);
    const mediaChannels = createInitialMediaChannels(state.voterParty, state.playerIssues);
  ...
  runSimulation: () => {
    ...
    const newPolling: Record<string, StatePollingProjection> = {};
    let totalDelegates = 0;
    let sumPlayer = 0;
    let sumRival = 0;
    let sumUndecided = 0;
    let primaryWeightTotal = 0;
    const primaryFieldTotals = new Map<string, PrimaryFieldShare>();
    ...
    const activeGeneralOpponent = state.generalOpponent
      ?? SimulationEngine.createGeneralOpponentAI(state.difficulty, state.voterParty, state.states, state.scenarioId);
    ...
    for (const s of state.states) {
      ...
      const playerMediaEffect = getMediaStateEffect(s, state.mediaChannels);
      const poll = state.gamePhase === 'primary'
        ? SimulationEngine.generatePrimaryFieldProjection(
            ...
            playerMediaEffect,
            Object.fromEntries(state.rivalAIs.map((entry) => [entry.id, SimulationEngine.getRivalMediaEffect(entry, s, 'primary')])),
            staffDiv,
            visitMult,
            state.playerIssues,
            state.voterParty,
            state.playerName,
            state.playerHomeRegion
          )
        : SimulationEngine.generateStatePolling(
            ...
            playerMediaEffect,
            SimulationEngine.getRivalMediaEffect(rival, s, 'general'),
            staffDiv,
            visitMult,
            state.playerIssues,
            state.voterParty,
      ...
      if (state.gamePhase === 'primary' && isPrimaryStateProjection(poll)) {
        const contestWeight = state.voterParty === 'Democrat' ? s.demDelegates : s.repDelegates;
        primaryWeightTotal += contestWeight;
        buildPrimaryFieldStanding(state.playerName, poll).forEach((candidate) => {
          const existing = primaryFieldTotals.get(candidate.candidateId);
          if (existing) {
            existing.share += candidate.share * contestWeight;
            return;
          }

          primaryFieldTotals.set(candidate.candidateId, {
            ...candidate,
            share: candidate.share * contestWeight
          });
        });
      }
    }
    ...
    const primaryFieldAverages = primaryWeightTotal > 0
      ? [...primaryFieldTotals.values()]
          .map((candidate) => ({
            ...candidate,
            share: candidate.share / primaryWeightTotal
          }))
    ...
    set((currentState) => {
      ...
      const fieldHistory = [...currentState.primaryFieldHistory];
      if (state.gamePhase === 'primary' && primaryFieldAverages.length > 0) {
        const existingFieldWeek = fieldHistory.findIndex((entry) => entry.week === nationalAvg.week);
        const nextEntry = {
          week: nationalAvg.week,
          standings: primaryFieldAverages
        };
        if (existingFieldWeek >= 0) {
          fieldHistory[existingFieldWeek] = nextEntry;
        } else {
          fieldHistory.push(nextEntry);
        }
      }
      return {
        pollingData: newPolling,
        nationalPollingHistory: history,
        primaryFieldAverages,
        primaryFieldHistory: fieldHistory
      };
    });
  },
  ...
  buildFieldOffice: (stateName) => {
    ...
    if (currentOperation.officeLevel >= 3) return false;
      ...
      const officeStateCount = Object.values(nextFieldOperations).filter((operation) => operation.officeLevel > 0).length;
      ...
      if (officeStateCount >= 8) {
        unlockAchievement('ACH_FIELD_MACHINE');
      }
      get().runSimulation();
      return true;
    },
  ...
  hireStaff: (staffId: string, cost: number) => {
    ...
    if (state.budget >= cost && !state.hiredStaff.includes(staffId)) {
      const nextStaff = [...state.hiredStaff, staffId];
      set({
        budget: state.budget - cost,
        hiredStaff: nextStaff,
        activityLog: [...state.activityLog, { week: state.currentWeek, message: `Hired new staff member.`, type: 'positive' }]
      });
      if (nextStaff.length >= 3) {
        unlockAchievement('ACH_FULL_STAFF');
      }
      get().runSimulation();
      return true;
    }
  ...
  selectVP: (vp: VPCandidate) => {
    ...
    for (const [key, val] of Object.entries(vp.bonuses) as Array<[keyof PlayerDemographics, number | undefined]>) {
      ...
      set({
        ...
        budget: Math.max(0, state.budget + vp.budgetBonus),
        volunteerReserve: Math.min(2400, state.volunteerReserve + vp.volunteerBonus),
        activityLog: capLog([
          ...state.activityLog,
          {
            week: state.currentWeek,
            message: `Selected ${vp.name} as running mate. ${vp.title} now joins the ticket.`,
            type: 'positive'
          },
          {
            week: state.currentWeek,
            message: `Ticket effect: +${vp.momentumBonus} momentum, +${vp.trustBonus} trust, +${Math.round(vp.budgetBonus / 1000)}K launch money, +${vp.volunteerBonus} volunteers.`,
            type: 'info'
          }
        ])
      });
      unlockAchievement('ACH_RUNNING_MATE');
      get().runSimulation();
    },

  fundraiseFromBloc: (blocId) => {
    const state = get();
    const target = state.donorBlocs.find((bloc) => bloc.id === blocId);
    if (!target) return false;
    if (state.stamina < target.staminaCost) return false;
    if (state.activeDebate || state.activeConvention || state.activeElectionNight) return false;

    const result = askDonorBloc(state.donorBlocs, blocId, state.playerIdeology, state.playerIssues, state.voterParty);
    if (!result) return false;

    set({
      donorBlocs: result.blocs,
      budget: state.budget + result.amount,
      publicTrust: clampStat(state.publicTrust + result.trustDelta),
      momentum: clampStat(state.momentum + result.momentumDelta),
      stamina: Math.max(0, state.stamina - result.staminaCost),
      activityLog: capLog([
        ...state.activityLog,
        {
          week: state.currentWeek,
          message: result.summary,
          type: result.trustDelta >= 0 ? 'positive' : 'event'
        }
      ])
    });
    get().runSimulation();
  ...
  investInMedia: (channelId) => {
    const state = get();
    const channel = getMediaChannelById(state.mediaChannels, channelId);
    if (!channel || state.budget < channel.investmentCost) return false;

    const nextChannels = investInMediaChannel(state.mediaChannels, channelId);
    const intensityGain = (nextChannels.find((entry) => entry.id === channelId)?.intensity ?? 0) - channel.intensity;
    set({
      mediaChannels: nextChannels,
      budget: state.budget - channel.investmentCost,
      momentum: clampStat(state.momentum + (channelId === 'earned_media' ? 2 : channelId === 'digital' ? 1 : 0)),
      publicTrust: clampStat(state.publicTrust + (channelId === 'rapid_response' ? 1 : 0)),
      activityLog: capLog([
        ...state.activityLog,
        {
          week: state.currentWeek,
          message: `Expanded ${channel.name.toLowerCase()} by ${intensityGain} intensity for ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(channel.investmentCost)}.`,
          type: channelId === 'rapid_response' ? 'positive' : 'info'
        }
      ])
    });
    get().runSimulation();
  ...
  commissionResearch: (rivalId) => {
    const state = get();
    if (state.activeEvent || state.activeDebate || state.activeConvention || state.activeElectionNight) return false;

    const target = state.rivalAIs.find((rival) => rival.id === rivalId)
      ?? (state.generalOpponent?.id === rivalId ? state.generalOpponent : null);
    if (!target) return false;

    const file = state.oppositionResearch[rivalId] ?? createOppositionResearchBoard([target])[rivalId];
    const researchCost = state.gamePhase === 'general' ? 105000 : 85000;
    const staminaCost = 6;
    if (state.budget < researchCost || state.stamina < staminaCost) return false;
    if (file.lastResearchWeek === state.currentWeek) return false;

    const researchPressure = getNationalResearchPressure(state.campaignSpending);
    const result = commissionOppositionResearch(file, target, state.currentWeek, state.publicTrust, researchPressure);
    const updatedBoard = {
    ...
    const newLog = [...state.activityLog, {
      week: state.currentWeek,
      message: result.summary,
      type: result.discoveredLead ? 'positive' as const : 'info' as const
    }];

    if (result.discoveredLead) {
      newLog.push({
        week: state.currentWeek,
        message: `War room brief: ${result.discoveredLead.title} is now ready to deploy against ${target.name}.`,
        type: result.discoveredLead.severity >= 70 ? 'positive' : 'info'
      });
    }

    set({
      budget: Math.max(0, state.budget - researchCost),
      stamina: Math.max(0, state.stamina - staminaCost),
      publicTrust: clampStat(state.publicTrust + result.trustDelta),
      momentum: clampStat(state.momentum + result.momentumDelta),
      oppositionResearch: updatedBoard,
  ...
  releaseResearchLead: (rivalId, leadId) => {
    const state = get();
    if (state.activeEvent || state.activeDebate || state.activeConvention || state.activeElectionNight) return false;

    const target = state.rivalAIs.find((rival) => rival.id === rivalId)
      ?? (state.generalOpponent?.id === rivalId ? state.generalOpponent : null);
    const file = state.oppositionResearch[rivalId];
    const lead = file?.leads.find((entry) => entry.id === leadId && entry.status === 'active');
    if (!target || !file || !lead) return false;
    if (file.lastReleaseWeek === state.currentWeek) return false;

    const researchPressure = getNationalResearchPressure(state.campaignSpending);
    const rapidResponseIntensity = getRapidResponseIntensity(state.mediaChannels);
    const result = releaseOppositionHit(
      file,
      lead,
      target,
      state.publicTrust,
      rapidResponseIntensity,
      researchPressure,
      state.currentWeek
    );
    ...
    const nextDonorBlocs = applyDonorEffects(state.donorBlocs, [{
      blocId: result.donorLane,
      relationshipDelta: result.outcome === 'backfire' ? -2 : 4,
      energyDelta: result.outcome === 'breakthrough' ? 10 : result.outcome === 'contained' ? 5 : -5
    }]);
    const nextMediaChannels = applyMediaEffects(state.mediaChannels, result.mediaBoosts);
    const updatedRivals = state.rivalAIs.map((rival) => {
      if (rival.id !== rivalId) return rival;
      return {
        ...rival,
        trust: clampStat(rival.trust + result.rivalTrustDelta),
        momentum: clampStat(rival.momentum + result.rivalMomentumDelta),
        budget: Math.max(0, rival.budget + result.rivalBudgetDelta)
      };
    });
    const updatedGeneralOpponent = state.generalOpponent?.id === rivalId
      ? {
          ...state.generalOpponent,
          trust: clampStat(state.generalOpponent.trust + result.rivalTrustDelta),
          momentum: clampStat(state.generalOpponent.momentum + result.rivalMomentumDelta),
          budget: Math.max(0, state.generalOpponent.budget + result.rivalBudgetDelta)
        }
      : state.generalOpponent;

    const newLog = [...state.activityLog, {
      week: state.currentWeek,
      message: `Released ${lead.title} against ${target.name}.`,
      type: result.outcome === 'backfire' ? 'negative' as const : 'event' as const
    }, {
      week: state.currentWeek,
      message: result.summary,
      type: result.outcome === 'backfire' ? 'negative' as const : result.outcome === 'breakthrough' ? 'positive' as const : 'info' as const
    }];

    set({
      publicTrust: clampStat(state.publicTrust + result.playerTrustDelta),
      momentum: clampStat(state.momentum + result.playerMomentumDelta),
      volunteerReserve: Math.max(0, state.volunteerReserve + result.volunteerDelta),
      oppositionResearch: nextBoard,
      donorBlocs: nextDonorBlocs,
      mediaChannels: nextMediaChannels,
  ...
  saveGame: (slot = 0) => {
    ...
    const saveData: PersistedGameState = {
      ...
      playerHomeRegion: state.playerHomeRegion,
      budget: state.budget,
      ...
      donorBlocs: state.donorBlocs,
      mediaChannels: state.mediaChannels,
      oppositionResearch: state.oppositionResearch,
      states: state.states,
      ...
      primaryFieldAverages: state.primaryFieldAverages,
      primaryFieldHistory: state.primaryFieldHistory,
    ...
    localStorage.setItem(`politisim_save_${slot}`, serialized);
    if (typeof window !== 'undefined' && window.electron) {
      void window.electron.writeCloudFile(getCloudSaveFileName(slot), serialized);
    }
  ...
  loadGame: (slot = 0) => {
    ...
    if (saveDataStr) {
      try {
        ...
        const restoredCalendar = CampaignDataParser.generateCalendar(parsed.scenarioElectionYear ?? initialState.scenarioElectionYear);
        ...
        const restoredParty = parsed.voterParty ?? initialState.voterParty;
        const restoredIdeology = parsed.playerIdeology ?? initialState.playerIdeology;
        const restoredIssues = parsed.playerIssues ?? initialState.playerIssues;
        ...
        const normalizedGeneralOpponent = parsed.generalOpponent
          ? {
              ...SimulationEngine.createGeneralOpponentAI(parsed.difficulty ?? initialState.difficulty, parsed.voterParty ?? initialState.voterParty, restoredStates, parsed.scenarioId ?? initialState.scenarioId),
        ...
        const normalizedDonorBlocs = parsed.donorBlocs?.length
          ? parsed.donorBlocs
          : createInitialDonorBlocs(restoredIdeology, restoredIssues, restoredParty);
        const normalizedMediaChannels = parsed.mediaChannels?.length
          ? parsed.mediaChannels
          : createInitialMediaChannels(restoredParty, restoredIssues);
        const defaultVPBench = getScenarioVPCandidates(parsed.scenarioId ?? initialState.scenarioId, restoredParty);
        const normalizedVPPick = parsed.vpPick
          ? (() => {
              const matched = defaultVPBench.find((candidate) => candidate.id === (parsed.vpPick as Partial<VPCandidate>).id || candidate.name === parsed.vpPick?.name);
              return matched ? { ...matched, ...parsed.vpPick } : null;
            })()
          : null;
        const normalizedResearch = normalizeOppositionResearch(
          parsed.oppositionResearch,
          [
            ...normalizedRivals,
            ...(normalizedGeneralOpponent ? [normalizedGeneralOpponent] : [])
          ]
        );
        ...
        set({
          ...
          ...parsed,
          hasStarted: true,
          scenarioElectionYear: parsed.scenarioElectionYear ?? initialState.scenarioElectionYear,
          calendar: restoredCalendar,
          calendarPhase: restoredCalendarPhase,
          playerIdeology: restoredIdeology,
          playerHomeRegion: parsed.playerHomeRegion ?? initialState.playerHomeRegion,
          playerIssues: restoredIssues,
          voterParty: restoredParty,
          activeEvent: null,
          ...
          donorBlocs: normalizedDonorBlocs,
          mediaChannels: normalizedMediaChannels,
          oppositionResearch: normalizedResearch,
          ...
          primaryFieldAverages: parsed.primaryFieldAverages ?? [],
          primaryFieldHistory: parsed.primaryFieldHistory ?? [],
          vpPick: normalizedVPPick,
  ...
  resetGame: () => {
    set({
      ...initialState,
      rivalAIs: SimulationEngine.createPrimaryRivals('normal', initialState.voterParty, initialState.states, initialState.scenarioId),
      generalOpponent: null,
      primaryResults: {},
      ...
      donorBlocs: createInitialDonorBlocs(initialState.playerIdeology, initialState.playerIssues, initialState.voterParty),
      mediaChannels: createInitialMediaChannels(initialState.voterParty, initialState.playerIssues),
      oppositionResearch: {}
    });
  },
  ...
  advanceWeek: () => {
    ...
    const updatedDonorBlocs = recoverDonorBlocs(
      state.donorBlocs,
    ...
    const updatedMediaChannels = applyWeeklyMediaDecay(state.mediaChannels);
    const mediaSummary = getMediaSummary(updatedMediaChannels);
    const researchPressure = getNationalResearchPressure(state.campaignSpending);
    let updatedOppositionResearch = ageOppositionResearch(state.oppositionResearch, nextWeek);
    ...
    if (researchPressure >= 6) {
      newLog.push({
        week: nextWeek,
        message: `War room note: your opposition desk is carrying enough active research pressure to shape the next contrast fight.`,
        type: 'info'
      });
    }
    ...
    updatedRivals = updatedRivals.map((rival) => {
      const beat = SimulationEngine.resolveRivalCampaignBeat(rival, newCalendarPhase === 'general' ? 'general' : 'primary', newTrust);
      if (!beat) return rival;
      newLog.push({
        week: nextWeek,
    ...
    if (nextGamePhase === 'general' && updatedGeneralOpponent) {
      ...
      const generalBeat = SimulationEngine.resolveRivalCampaignBeat(updatedGeneralOpponent, 'general', newTrust);
      if (generalBeat) {
        updatedGeneralOpponent = generalBeat.rival;
        newLog.push({
          week: nextWeek,
          message: generalBeat.message,
          type: generalBeat.type
        });
      }
    }
    ...
    if (newCalendarPhase === 'primary' && nextWeek % 2 === 0) {
      ...
      let playerContestWinsThisWeek = 0;
      ...
      for (const contestState of uncontested) {
        const projection: PrimaryStateProjection = SimulationEngine.generatePrimaryFieldProjection(
          ...
          getMediaStateEffect(contestState, updatedMediaChannels),
          Object.fromEntries(updatedRivals.map((entry) => [entry.id, SimulationEngine.getRivalMediaEffect(entry, contestState, 'primary')])),
          contestStaffDiv,
          contestVisitMult,
          state.playerIssues,
          state.voterParty,
        ...
        if (raceLeader?.candidateId === 'player') {
          playerContestWinsThisWeek += 1;
        }
      ...
      const playerWinStats = getCandidateWinStats(updatedPrimaryResults, 'player', nextWeek);
      if (playerWinStats.stateWins >= 1) {
        unlockAchievement('ACH_FIRST_STATE_WIN');
      }
      if (playerContestWinsThisWeek >= 3) {
        unlockAchievement('ACH_SUPER_TUESDAY');
      }

      const suspension = evaluatePrimarySuspension(
        updatedRivals,
        updatedPrimaryResults,
        updatedEndorsements,
        state.playerIdeology,
        nextWeek,
      ...
      if (suspension) {
        updatedRivals = updatedRivals.map((rival) => {
          if (rival.id === suspension.withdrawingRival.id) {
            return {
              ...rival,
              status: 'withdrawn',
              momentum: Math.max(12, rival.momentum - 10),
              trust: Math.max(24, rival.trust - 8),
              endorsedCandidateId: suspension.endorsementTargetId
            };
          }

          if (rival.id === suspension.endorsementTargetId) {
            return {
              ...rival,
              supportBase: rival.supportBase + suspension.supportTransfer,
              momentum: Math.min(100, rival.momentum + suspension.momentumTransfer),
              trust: Math.min(100, rival.trust + suspension.trustTransfer)
            };
          }
        ...
        if (suspension.endorsementTargetId === 'player') {
          endorsementMomentumBonus += suspension.momentumTransfer;
          newTrust = clampStat(newTrust + suspension.trustTransfer);
          newLog.push({
            week: nextWeek,
            message: `${suspension.withdrawingRival.name} suspended their campaign after ${suspension.reason} and endorsed you, reframing the race around your coalition.`,
            type: 'positive'
          });
        } else if (suspension.endorsementTargetId) {
          const endorsedRival = updatedRivals.find((rival) => rival.id === suspension.endorsementTargetId);
          newLog.push({
            week: nextWeek,
            message: `${suspension.withdrawingRival.name} suspended their campaign after ${suspension.reason} and lined up behind ${endorsedRival?.name ?? 'another rival'}.`,
            type: endorsedRival?.id === getLeadRival(updatedRivals, state.difficulty).id ? 'negative' : 'info'
          });
        } else {
          newLog.push({
            week: nextWeek,
            message: `${suspension.withdrawingRival.name} suspended their campaign after ${suspension.reason}, but stopped short of endorsing anyone immediately.`,
            type: 'event'
          });
        }
      }
    ...
    if ((newCalendarPhase === 'primary' || newCalendarPhase === 'convention') && nextGamePhase === 'primary') {
      ...
      const playerCoalition = getCandidateEndorsementSummary(updatedEndorsements, 'player');
      if (playerCoalition.count >= 1) {
        unlockAchievement('ACH_FIRST_ENDORSEMENT');
      }
      if (playerCoalition.count >= 4 || playerCoalition.prestige >= 12) {
        unlockAchievement('ACH_COALITION_BUILDER');
      }
    }
    ...
    if (phaseChanged && newCalendarPhase === 'convention') {
      ...
      if (playerWonOutright) {
        ...
        updatedGeneralOpponent = SimulationEngine.createGeneralOpponentAI(state.difficulty, state.voterParty, state.states, state.scenarioId);
    ...
    updatedOppositionResearch = normalizeOppositionResearch(
      updatedOppositionResearch,
      [
        ...updatedRivals,
        ...(updatedGeneralOpponent ? [updatedGeneralOpponent] : [])
      ]
    );
    ...
    if (mediaSummary.strongestChannels.length > 0) {
      newLog.push({
        week: nextWeek,
        message: `Media posture: ${mediaSummary.strongestChannels.map((channel) => channel.name).join(', ')} remained your loudest channels this week.`,
        type: 'info'
      });
    }
    ...
    if (mediaOverhead > 0) {
      newLog.push({
        week: nextWeek,
        message: `Media maintenance: -${(mediaOverhead / 1000).toFixed(0)}K to hold your broadcast, digital, and rapid-response footprint.`,
        type: 'negative'
      });
    }
    ...
    if (nextGamePhase !== 'ended' && !nextActiveConvention && !nextActiveElectionNight) {
      if (scheduledDebate) {
        ...
        const baseDebate = createActiveDebate(scheduledDebate, state.playerName, rivalNames);
        const debate = {
          ...baseDebate,
          participants: buildDebateParticipants(baseDebate, state.playerIssues, updatedRivals, updatedGeneralOpponent)
        };
        ...
        const phaseEvents = [
          ...POSSIBLE_EVENTS,
          ...getScenarioEventDeck(state.scenarioId, eventPhase, 'positive')
    ...
    set({
      ...
      momentum: Math.max(0, Math.min(100, state.momentum - momentumLoss + conventionBounce + endorsementMomentumBonus + mediaSummary.momentumDelta)),
      stamina: newStamina,
      budget: nextBudget,
      ...
      donorBlocs: updatedDonorBlocs,
      mediaChannels: updatedMediaChannels,
      oppositionResearch: updatedOppositionResearch,
      ...
      publicTrust: clampStat(newTrust),
  ...
  resolveEvent: (choiceIndex: number) => {
    ...
    if (!choice) return;
    const nextDonorBlocs = applyDonorEffects(state.donorBlocs, choice.donorEffects);
    const nextMediaChannels = applyMediaEffects(state.mediaChannels, choice.mediaEffects);
    const rivalEffects = applyRivalEventEffects(choice, state.rivalAIs, state.generalOpponent);
    const newLog = [...state.activityLog, {
      week: state.currentWeek,
    ...
    if (choice.logLine) {
      newLog.push({
        week: state.currentWeek,
        message: choice.logLine,
        type: choice.trustEffect >= 0 || choice.momentumEffect >= 0 ? 'positive' : 'info'
      });
    }
    ...
    set({
      ...
      stamina: clampStat(state.stamina + (choice.staminaEffect ?? 0)),
      volunteerReserve: Math.max(0, state.volunteerReserve + (choice.volunteerEffect ?? 0)),
      donorBlocs: nextDonorBlocs,
      mediaChannels: nextMediaChannels,
      rivalAIs: rivalEffects.rivalAIs,
      generalOpponent: rivalEffects.generalOpponent,
      activeEvent: null,
      activityLog: capLog(newLog)
    });
    get().runSimulation();
  ...
  answerDebateQuestion: (choiceIndex: number) => {
    ...
    const answerRecord = createDebateAnswerRecord(debate, choiceIndex);
    ...
    set({
      activeDebate: {
        ...
        answerHistory: answerRecord ? [...debate.answerHistory, answerRecord] : debate.answerHistory
      },
  ...
  advanceDebate: () => {
    ...
    if (nextQuestionIndex >= debate.questions.length) {
      const featuredRival = debate.phase === 'general'
        ? state.generalOpponent ?? SimulationEngine.createGeneralOpponentAI(state.difficulty, state.voterParty, state.states, state.scenarioId)
        : getLeadRival(state.rivalAIs, state.difficulty);
      let updatedRivals = state.rivalAIs;
      let updatedGeneralOpponent = state.generalOpponent;
      let playerTrust = state.publicTrust;
      let playerMomentum = state.momentum;
      let styleAftermath = '';

      if (featuredRival.debateStyle === 'attacker') {
        updatedRivals = updatedRivals.map((rival) => rival.id === featuredRival.id ? { ...rival, momentum: clampStat(rival.momentum + 3), trust: clampStat(rival.trust + 1) } : rival);
        if (updatedGeneralOpponent?.id === featuredRival.id) {
          updatedGeneralOpponent = { ...updatedGeneralOpponent, momentum: clampStat(updatedGeneralOpponent.momentum + 3), trust: clampStat(updatedGeneralOpponent.trust + 1) };
        }
        playerTrust = clampStat(playerTrust - 1);
        styleAftermath = `${featuredRival.name}'s attack-heavy night kept the spin room focused on conflict and helped that campaign hold the cameras.`;
      } else if (featuredRival.debateStyle === 'reassurer') {
        updatedRivals = updatedRivals.map((rival) => rival.id === featuredRival.id ? { ...rival, trust: clampStat(rival.trust + 3), momentum: clampStat(rival.momentum + 1) } : rival);
        if (updatedGeneralOpponent?.id === featuredRival.id) {
          updatedGeneralOpponent = { ...updatedGeneralOpponent, trust: clampStat(updatedGeneralOpponent.trust + 3), momentum: clampStat(updatedGeneralOpponent.momentum + 1) };
        }
        styleAftermath = `${featuredRival.name}'s calmer style played well with late deciders looking for a steadier hand.`;
      } else if (featuredRival.debateStyle === 'wonk') {
        updatedRivals = updatedRivals.map((rival) => rival.id === featuredRival.id ? { ...rival, trust: clampStat(rival.trust + 2), momentum: clampStat(rival.momentum + 1) } : rival);
        if (updatedGeneralOpponent?.id === featuredRival.id) {
          updatedGeneralOpponent = { ...updatedGeneralOpponent, trust: clampStat(updatedGeneralOpponent.trust + 2), momentum: clampStat(updatedGeneralOpponent.momentum + 1) };
        }
        styleAftermath = `${featuredRival.name}'s policy-heavy performance impressed pundits and gave that campaign a competence bump.`;
      } else if (featuredRival.debateStyle === 'outsider') {
        updatedRivals = updatedRivals.map((rival) => rival.id === featuredRival.id ? { ...rival, momentum: clampStat(rival.momentum + 4), trust: clampStat(rival.trust - 1) } : rival);
        if (updatedGeneralOpponent?.id === featuredRival.id) {
          updatedGeneralOpponent = { ...updatedGeneralOpponent, momentum: clampStat(updatedGeneralOpponent.momentum + 4), trust: clampStat(updatedGeneralOpponent.trust - 1) };
        }
        playerMomentum = clampStat(playerMomentum - 1);
        styleAftermath = `${featuredRival.name} turned the debate into a spectacle, giving the campaign a loud momentum jolt even with uneven reviews.`;
      } else {
        updatedRivals = updatedRivals.map((rival) => rival.id === featuredRival.id ? { ...rival, momentum: clampStat(rival.momentum + 1), trust: clampStat(rival.trust - 1) } : rival);
        if (updatedGeneralOpponent?.id === featuredRival.id) {
          updatedGeneralOpponent = { ...updatedGeneralOpponent, momentum: clampStat(updatedGeneralOpponent.momentum + 1), trust: clampStat(updatedGeneralOpponent.trust - 1) };
        }
        styleAftermath = `${featuredRival.name} dodged clean answers often enough to irritate moderators, but still bought some time with the base.`;
      }
      const aftermathSpec = buildDebateAftermath(debate, state.debateStanding, state.playerName, featuredRival.name);
      const debateAftermath: CampaignEvent = {
        title: aftermathSpec.title,
        description: `${aftermathSpec.description} ${aftermathSpec.headline}`,
        phase: debate.phase,
        choices: aftermathSpec.choices.map((choice) => ({
          text: choice.text,
          moneyEffect: choice.moneyEffect,
          momentumEffect: choice.momentumEffect,
          trustEffect: choice.trustEffect,
          volunteerEffect: choice.volunteerEffect,
          donorEffects: choice.donorEffects,
          mediaEffects: choice.mediaEffects,
          rivalId: featuredRival.id,
          rivalMomentumEffect: choice.rivalMomentumEffect,
          rivalTrustEffect: choice.rivalTrustEffect,
          rivalBudgetEffect: choice.rivalBudgetEffect,
          tags: choice.tags,
          logLine: choice.logLine
        }))
      };
      ...
      newLog.push({
        week: state.currentWeek,
        message: styleAftermath,
        type: featuredRival.debateStyle === 'reassurer' || featuredRival.debateStyle === 'wonk' ? 'info' : 'negative'
      });
      newLog.push({
        week: state.currentWeek,
        message: `Post-debate fallout: ${debateAftermath.title}.`,
        type: aftermathSpec.performance === 'shaky' ? 'negative' : 'event'
      });
      unlockAchievement('ACH_DEBATE_STAGE');
  ...
  previewDebate: (phase: DebatePhase) => {
    ...
    const rivalNames = phase === 'general'
      ? [state.generalOpponent?.name ?? SimulationEngine.createGeneralOpponentAI(state.difficulty, state.voterParty, state.states, state.scenarioId).name]
      : getOrderedRivals(state.rivalAIs).map((rival) => rival.name);
    const baseDebate = createActiveDebate(debateEntry, state.playerName, rivalNames);
    const previewDebate = {
      ...baseDebate,
      participants: buildDebateParticipants(baseDebate, state.playerIssues, state.rivalAIs, state.generalOpponent)
    };
  ...
  advanceConvention: () => {
    ...
    if (shouldResolve) {
      ...
      if (playerWonConvention) {
        const updatedGeneralOpponent = SimulationEngine.createGeneralOpponentAI(state.difficulty, state.voterParty, state.states, state.scenarioId);
        const totalEV = state.states.reduce((sum, contestState) => sum + contestState.delegatesOrEV, 0);
        const nextEVTarget = Math.floor(totalEV / 2) + 1;
        unlockAchievement('ACH_BROKERED_CONVENTION');
        newLog.push({
          week: state.currentWeek,
        ...
        const oppositionResearch = normalizeOppositionResearch(
          state.oppositionResearch,
          [...state.rivalAIs, updatedGeneralOpponent]
        );
