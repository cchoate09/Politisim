...
async function runSmokeTest(win) {
  ...
  try {
    ...
    const smokeResult = await win.webContents.executeJavaScript(`(async () => {
      ...
      await waitFor(() => Boolean(window.__POLITISIM_SMOKE__), 'smoke bridge');

      if (!window.electron.runtimeFlags?.isSmokeTest) {
        throw new Error('Runtime flags did not report smoke mode to the renderer');
      }

      const smokeBridge = window.__POLITISIM_SMOKE__;
      if (!smokeBridge) {
        throw new Error('Smoke bridge was not installed on the renderer window');
      }

      await smokeBridge.clearImportedScenarios();
      const catalogBeforeImport = await smokeBridge.getScenarioCatalog();
      if (catalogBeforeImport.length < 5) {
        throw new Error(\`Scenario catalog unexpectedly small before import: \${catalogBeforeImport.length}\`);
      }
      ...
      const vanillaSummary = smokeBridge.getStateSummary();
      if (vanillaSummary.scenarioId !== 'vanilla' || vanillaSummary.scenarioElectionYear !== 2024) {
        throw new Error(\`Unexpected vanilla state summary: \${JSON.stringify(vanillaSummary)}\`);
      }

      const debatePreview = smokeBridge.runPreviewDebate('primary');
      if (!debatePreview.debateId || debatePreview.questionCount < 10) {
        throw new Error(\`Debate preview did not open a full debate: \${JSON.stringify(debatePreview)}\`);
      }

      const debateResolution = smokeBridge.completeActiveDebate(new Array(10).fill(0));
      if (!debateResolution.aftermathTitle || !debateResolution.achievements.includes('ACH_DEBATE_STAGE')) {
        throw new Error(\`Debate completion did not unlock the expected achievement: \${JSON.stringify(debateResolution)}\`);
      }
      smokeBridge.resolveActiveEvent(0);

      const restorationSummary = await smokeBridge.bootstrapCampaign({
        scenarioId: 'restoration_2012',
        playerName: 'Regression Candidate',
        party: 'Republican',
        difficulty: 'hard',
        issues: ['Economy', 'National Security', 'Education'],
        homeRegion: 'Midwest'
      });
      if (restorationSummary.scenarioId !== 'restoration_2012' || restorationSummary.scenarioElectionYear !== 2012) {
        throw new Error(\`Restoration scenario bootstrap failed: \${JSON.stringify(restorationSummary)}\`);
      }

      const slotRoundTrip = smokeBridge.saveAndReloadSlot(1);
      if (slotRoundTrip.after.scenarioId !== 'restoration_2012' || slotRoundTrip.slotSummary?.slot !== 1) {
        throw new Error(\`Save/load round trip failed: \${JSON.stringify(slotRoundTrip)}\`);
      }

      const importedStates = vanillaStates.map((state) => ({
        ...state,
        date: \`2036-\${state.date.slice(5)}\`
      }));
      const importedScenario = await smokeBridge.importScenario({
        manifest: {
          id: 'smoke-future-path',
          name: 'Smoke Future Path',
          yearLabel: '2036',
          electionYear: 2036,
          tagline: 'Synthetic imported scenario for packaged QA.',
          description: 'Generated by the packaged launch-candidate smoke suite to verify imported scenario handling.',
          challenge: 'Competitive',
          focus: ['Smoke QA', 'Imported content', 'Regression coverage'],
          featuredStates: ['Arizona', 'Michigan'],
          specialRules: ['Synthetic imported validation pass'],
      ...
      if (importedScenario.status === 'invalid' || importedScenario.catalogCount !== catalogBeforeImport.length + 1) {
        throw new Error(\`Imported scenario failed catalog validation: \${JSON.stringify(importedScenario)}\`);
      }

      const importedSummary = await smokeBridge.bootstrapCampaign({
        scenarioId: importedScenario.importedId,
        playerName: 'Imported Candidate',
        party: 'Democrat',
        difficulty: 'normal',
        issues: ['Climate Change', 'Healthcare', 'Education'],
        homeRegion: 'West'
      });
      if (importedSummary.scenarioId !== importedScenario.importedId || importedSummary.scenarioElectionYear !== 2036) {
        throw new Error(\`Imported scenario bootstrap failed: \${JSON.stringify(importedSummary)}\`);
      }

      const openedConvention = smokeBridge.openSyntheticConvention();
      if (openedConvention.ballot !== 1 || openedConvention.freeDelegates <= 0) {
        throw new Error(\`Synthetic convention did not open correctly: \${JSON.stringify(openedConvention)}\`);
      }

      const resolvedConvention = smokeBridge.resolveSyntheticConvention();
      if (resolvedConvention.gamePhase !== 'general' || !resolvedConvention.achievements.includes('ACH_BROKERED_CONVENTION')) {
        throw new Error(\`Synthetic convention did not produce a nomination win: \${JSON.stringify(resolvedConvention)}\`);
      }

      const openedElectionNight = smokeBridge.openSyntheticElectionNight();
      if (openedElectionNight.targetEV < 270) {
        throw new Error(\`Synthetic election night opened with an invalid EV target: \${JSON.stringify(openedElectionNight)}\`);
      }

      const resolvedElectionNight = smokeBridge.resolveElectionNightToEnd();
      if (resolvedElectionNight.gamePhase !== 'ended' || !['general_win', 'general_loss'].includes(resolvedElectionNight.endReason)) {
        throw new Error(\`Election night did not resolve into an endgame state: \${JSON.stringify(resolvedElectionNight)}\`);
      }
      await waitFor(() => visibleText().includes('ELECTION NIGHT VICTORY') || visibleText().includes('CONCESSION NIGHT') || visibleText().includes('PRIMARY DEFEAT'), 'endgame screen');

      const removedScenario = await smokeBridge.removeScenario(importedScenario.importedId);
      if (removedScenario.catalogCount !== catalogBeforeImport.length) {
        throw new Error(\`Imported scenario removal failed: \${JSON.stringify(removedScenario)}\`);
      }
      ...
      return {
        ...
        launchCandidateChecks: {
          catalogCountBeforeImport: catalogBeforeImport.length,
          importedScenarioId: importedScenario.importedId,
          restorationScenarioYear: restorationSummary.scenarioElectionYear,
          importedScenarioYear: importedSummary.scenarioElectionYear,
          debateAftermathTitle: debateResolution.aftermathTitle,
          conventionResultPhase: resolvedConvention.gamePhase,
          electionNightEndReason: resolvedElectionNight.endReason,
          unlockedAchievements: resolvedElectionNight.achievements
        }
      };
    })()`, true);
